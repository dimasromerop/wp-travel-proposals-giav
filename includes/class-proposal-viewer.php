                        ...
                        <div class="service-group__grid">
                            <?php foreach ( $package_items as $item ) : ?>
                                ...
                                if ( $package_pricing_basis === 'per_room' ) {
                                    ...
                                    $room_single = (float) ( $item['package_unit_sell_price_individual'] ?? $item['package_room_single'] ?? $item['unit_sell_price_single_room'] ?? 0 );
                                    $room_supp = (float) ( $item['package_single_supplement_pvp'] ?? $item['package_room_single_supplement'] ?? $item['package_single_room_supplement_sell'] ?? 0 );
                                    if ( $room_double > 0 ) {
                                    ...
                                    $pp_single = (float) ( $item['package_unit_sell_price_individual'] ?? $item['package_pp_single'] ?? $item['unit_sell_price_individual'] ?? 0 );
                        ...
                        <div class="service-group__grid">
                            <?php foreach ( $other_items as $item ) : ?>
                                ...
                                <?php if ( ( $item['service_type'] ?? '' ) === 'package' && ( $package_pricing_basis === 'per_person' || $package_pricing_basis === '' ) ) : ?>
                                    ...
                                    <?php endif; ?>
                                <?php endif; ?>
                                <?php if ( ( $item['service_type'] ?? '' ) === 'package' && $package_pricing_basis === 'per_room' ) : ?>
                                    <?php
                                    $room_double = (float) ( $item['package_room_double'] ?? $item['unit_sell_price'] ?? 0 );
                                    ...
                                    $room_mode = (string) ( $item['package_single_room_mode'] ?? '' );
                                    $pricing_bits = [];
                                    if ( $room_double > 0 ) {
                                        $pricing_bits[] = sprintf( __( 'Precio por habitación (doble): %s', 'wp-travel-giav' ), self::format_currency_value( $room_double, $currency ) );
                                    }
                                    if ( $room_mode === 'supplement' ) {
                                        if ( $room_supp > 0 ) {
                                            $pricing_bits[] = sprintf( __( 'Suplemento habitación individual: %s', 'wp-travel-giav' ), self::format_currency_value( $room_supp, $currency ) );
                                        }
                                    } else {
                                        if ( $room_single > 0 ) {
                                            $pricing_bits[] = sprintf( __( 'Precio habitación individual: %s', 'wp-travel-giav' ), self::format_currency_value( $room_single, $currency ) );
                                        }
                                    }
                                    ?>
                                    <?php if ( ! empty( $pricing_bits ) ) : ?>
